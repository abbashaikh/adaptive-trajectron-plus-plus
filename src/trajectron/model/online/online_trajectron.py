...
class OnlineTrajectron(Trajectron):
    ...
    def set_environment(self, env, init_timestep=0):
        ...
        for timestep in range(init_timestep + 1):
            self.incremental_forward(
                self.env.scenes[0].get_clipped_input_dict(
                    timestep, self.hyperparams["state"]
                ),
                maps=None,
                run_models=False,
            )
    ...
    def incremental_forward(
        ...
        with torch.no_grad():
            ...
            newly_removed_nodes = (
                set(self.node_data.keys()) - set(self.removed_nodes.keys())
            ) - set(new_inputs_dict.keys())
            ...
            for node in self.node_data:
                node.overwrite_data(
                    self.node_data[node],
                    None,
                    forward_in_time_on_next_overwrite=(
            ...
            if self.hyperparams["dynamic_edges"] == "yes":
                new_nodes, removed_nodes, new_neighbors, removed_neighbors = (
                    new_scene_graph - self.scene_graph
                )
